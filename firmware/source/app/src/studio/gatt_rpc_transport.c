...
static ssize_t write_rpc_req(struct bt_conn *conn, const struct bt_gatt_attr *attr, const void *buf,
    ...
    if (ring_buf_space_get(rpc_buf) < len) {
        LOG_WRN("Dropping BLE RPC write (%u bytes): RX ring buffer full", len);
        return BT_GATT_ERR(BT_ATT_ERR_INSUFFICIENT_RESOURCES);
    }

    zmk_rpc_select_transport(ZMK_TRANSPORT_BLE);
    ...
    while (copied < len) {
        ...
        if (claim_len == 0) {
            LOG_WRN("BLE RPC RX claim failed after preflight (copied=%u/%u)", copied, len);
            return BT_GATT_ERR(BT_ATT_ERR_INSUFFICIENT_RESOURCES);
        }
