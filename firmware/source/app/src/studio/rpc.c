...
static K_MUTEX_DEFINE(rpc_listener_buf_mutex);
static zmk_studio_Response rpc_listener_resp;
static zmk_studio_Notification rpc_listener_notification;
...
static void handle_request(const zmk_studio_Request *req, zmk_studio_Response *resp) {
    ...
    if (!sub) {
        ...
        *resp = ZMK_RPC_RESPONSE(meta, simple_error, zmk_meta_ErrorConditions_RPC_NOT_FOUND);
        return;
    }

    *resp = sub->func(sub, req);
...
static struct zmk_rpc_transport *find_transport(enum zmk_transport transport) {
...
void zmk_rpc_select_transport(enum zmk_transport transport) {
    ...
    struct zmk_rpc_transport *transport_impl = find_transport(transport);
    if (!transport_impl) {
        LOG_WRN("Failed to select RPC transport %d", transport);
        goto exit_select;
    }

    selected_transport = transport_impl;
...
static bool rpc_tx_buffer_write(pb_ostream_t *stream, const uint8_t *buf, size_t count) {
    ...
    do {
        ...
        if (claim_len == 0) {
            if (++no_space_retries > 1000) {
                LOG_ERR("RPC TX ring stayed full while encoding response; aborting frame");
                return false;
            }
            k_sleep(K_MSEC(1));
            continue;
...
static int send_response(const zmk_studio_Response *resp) {
    ...
    if (!selected_transport) {
        LOG_WRN("Dropping RPC response, no transport is active");
        goto exit;
    }
...
static void rpc_main(void) {
    static zmk_studio_Request req;
    static zmk_studio_Response resp;

    for (;;) {
        pb_istream_t stream = pb_istream_for_rx_ring_buf();
        req = (zmk_studio_Request)zmk_studio_Request_init_zero;
...
static void start_all_transports(void) {
    struct zmk_rpc_transport *default_transport = NULL;
...
static int studio_rpc_listener_cb(const zmk_event_t *eh) {
    ...
    if (rpc_notify) {
        k_mutex_lock(&rpc_listener_buf_mutex, K_FOREVER);
        rpc_listener_resp = (zmk_studio_Response)zmk_studio_Response_init_zero;
        rpc_listener_resp.which_type = zmk_studio_Response_notification_tag;
        rpc_listener_resp.type.notification = rpc_notify->notification;
        send_response(&rpc_listener_resp);
        k_mutex_unlock(&rpc_listener_buf_mutex);
        return ZMK_EV_EVENT_BUBBLE;
    }

    k_mutex_lock(&rpc_listener_buf_mutex, K_FOREVER);
    rpc_listener_notification = (zmk_studio_Notification)zmk_studio_Notification_init_zero;
    STRUCT_SECTION_FOREACH(zmk_rpc_event_mapper, mapper) {
        int ret = mapper->func(eh, &rpc_listener_notification);
        if (ret >= 0) {
            struct zmk_studio_rpc_notification notification_event = {
                .notification = rpc_listener_notification,
            };
            k_mutex_unlock(&rpc_listener_buf_mutex);
