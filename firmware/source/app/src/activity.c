...
static bool runtime_idle_enabled = (MAX_IDLE_MS > 0);
static uint32_t runtime_idle_timeout_ms = MAX_IDLE_MS;
#if IS_ENABLED(CONFIG_ZMK_SLEEP)
static bool runtime_sleep_enabled = false;
static uint32_t runtime_sleep_timeout_ms = MAX_SLEEP_MS;
#else
static bool runtime_sleep_enabled = false;
static uint32_t runtime_sleep_timeout_ms = 0;
#endif
static bool runtime_sleep_while_usb_powered = false;
...
bool zmk_activity_get_idle_enabled(void) { return runtime_idle_enabled; }
uint32_t zmk_activity_get_idle_timeout_ms(void) { return runtime_idle_timeout_ms; }
bool zmk_activity_get_sleep_enabled(void) { return runtime_sleep_enabled; }
uint32_t zmk_activity_get_sleep_timeout_ms(void) { return runtime_sleep_timeout_ms; }
bool zmk_activity_get_sleep_while_usb_powered(void) { return runtime_sleep_while_usb_powered; }
...
void zmk_activity_wake(void) { (void)note_activity(); }

void zmk_activity_set_idle_enabled(bool enabled) {
    runtime_idle_enabled = enabled;
    zmk_activity_wake();
}
...
#if IS_ENABLED(CONFIG_ZMK_SLEEP)
    runtime_sleep_enabled = enabled;
#else
    ARG_UNUSED(enabled);
    runtime_sleep_enabled = false;
#endif
    zmk_activity_wake();
}
...
#if IS_ENABLED(CONFIG_ZMK_SLEEP)
    runtime_sleep_timeout_ms = timeout_ms;
#else
    ARG_UNUSED(timeout_ms);
    runtime_sleep_timeout_ms = 0;
#endif
    zmk_activity_wake();
}

void zmk_activity_set_sleep_while_usb_powered(bool enabled) {
    runtime_sleep_while_usb_powered = enabled;
    zmk_activity_wake();
}
...
#if IS_ENABLED(CONFIG_ZMK_SLEEP)
    if (runtime_sleep_enabled && runtime_sleep_timeout_ms > 0 &&
        inactive_time > runtime_sleep_timeout_ms &&
        (runtime_sleep_while_usb_powered || !is_usb_power_present())) {
        ...
        if (runtime_idle_enabled && runtime_idle_timeout_ms > 0 &&
            inactive_time > runtime_idle_timeout_ms) {
