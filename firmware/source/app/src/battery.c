...
static uint32_t runtime_report_interval_s = CONFIG_ZMK_BATTERY_REPORT_INTERVAL;

uint8_t zmk_battery_state_of_charge(void) { return last_state_of_charge; }
uint32_t zmk_battery_get_report_interval_seconds(void) { return runtime_report_interval_s; }
...
static void zmk_battery_start_reporting() {
    if (device_is_ready(battery)) {
        k_timer_start(&battery_timer, K_NO_WAIT, K_SECONDS(runtime_report_interval_s));
    }
}

void zmk_battery_set_report_interval_seconds(uint32_t interval_seconds) {
    runtime_report_interval_s = CLAMP(interval_seconds, 10U, 3600U);
