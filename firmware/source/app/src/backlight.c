...
static bool preferred_on = IS_ENABLED(CONFIG_ZMK_BACKLIGHT_ON_START);
...
int zmk_backlight_toggle(void) { return preferred_on ? zmk_backlight_off() : zmk_backlight_on(); }

bool zmk_backlight_is_on(void) { return state.on; }
bool zmk_backlight_get_preferred_on(void) { return preferred_on; }

int zmk_backlight_set_brt(uint8_t brightness) {
    state.brightness = MIN(brightness, BRT_MAX);
    preferred_on = (state.brightness > 0);
    state.on = preferred_on;
    return zmk_backlight_update_and_save();
}

uint8_t zmk_backlight_get_brt(void) { return state.on ? state.brightness : 0; }
uint8_t zmk_backlight_get_preferred_brt(void) { return state.brightness; }
...
static int backlight_auto_state(bool new_state) {
    bool next_on = new_state && preferred_on;
...
static int backlight_event_listener(const zmk_event_t *eh) {
    ...
    if (as_zmk_activity_state_changed(eh)) {
        return backlight_auto_state(zmk_activity_get_state() == ZMK_ACTIVITY_ACTIVE);
    }
#endif

#if IS_ENABLED(CONFIG_ZMK_BACKLIGHT_AUTO_OFF_USB)
    if (as_zmk_usb_conn_state_changed(eh)) {
        return backlight_auto_state(zmk_usb_is_powered());
    }
#endif
