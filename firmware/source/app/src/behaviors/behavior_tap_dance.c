...
static int get_tap_dance_runtime_config(const struct device *dev,
    ...
    *config = (struct behavior_runtime_config){
        .type = BEHAVIOR_RUNTIME_CONFIG_TYPE_TAP_DANCE,
        .data.tap_dance =
            {
                .tapping_term_ms = cfg->tapping_term_ms,
                .binding_count = cfg->behavior_count,
            },
    };

    memcpy(config->data.tap_dance.bindings, cfg->behaviors,
           sizeof(struct zmk_behavior_binding) * cfg->behavior_count);
...
static int set_tap_dance_runtime_config(const struct device *dev,
    ...
    for (size_t i = 0; i < runtime->binding_count; i++) {
        if (!runtime->bindings[i].behavior_dev ||
            strcmp(runtime->bindings[i].behavior_dev, dev->name) == 0) {
            return -EINVAL;
        }

        int err = zmk_behavior_validate_binding(&runtime->bindings[i]);
        if (err < 0) {
            return err;
        }
    }
    ...
    memcpy(cfg->behaviors, runtime->bindings,
           sizeof(struct zmk_behavior_binding) * runtime->binding_count);
    cfg->behavior_count = runtime->binding_count;
