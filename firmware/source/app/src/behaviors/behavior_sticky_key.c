...
static int get_sticky_key_runtime_config(const struct device *dev,
    ...
    *config = (struct behavior_runtime_config){
        ...
        .data.sticky_key =
            {
                .release_after_ms = cfg->release_after_ms,
                .quick_release = cfg->quick_release,
                .lazy = cfg->lazy,
                .ignore_modifiers = cfg->ignore_modifiers,
                .behavior_dev = cfg->behavior.behavior_dev,
            },
    };
...
static int set_sticky_key_runtime_config(const struct device *dev,
    ...
    if (!zmk_behavior_get_binding(runtime->behavior_dev)) {
        return -ENOENT;
    }

    cfg->release_after_ms = runtime->release_after_ms;
    cfg->quick_release = runtime->quick_release;
    cfg->lazy = runtime->lazy;
    cfg->ignore_modifiers = runtime->ignore_modifiers;
    cfg->behavior.behavior_dev = (char *)runtime->behavior_dev;
    cfg->behavior.param1 = 0;
    cfg->behavior.param2 = 0;
...
#endif // IS_ENABLED(CONFIG_ZMK_BEHAVIOR_METADATA)
    .get_runtime_config = get_sticky_key_runtime_config,
    .set_runtime_config = set_sticky_key_runtime_config,
};
