...
static inline const char *flavor_str(enum behavior_hold_tap_flavor flavor) {
    switch (flavor) {
    case BEHAVIOR_HOLD_TAP_FLAVOR_HOLD_PREFERRED:
        return "hold-preferred";
    case BEHAVIOR_HOLD_TAP_FLAVOR_BALANCED:
        return "balanced";
    case BEHAVIOR_HOLD_TAP_FLAVOR_TAP_PREFERRED:
        return "tap-preferred";
    case BEHAVIOR_HOLD_TAP_FLAVOR_TAP_UNLESS_INTERRUPTED:
...
static int get_hold_tap_runtime_config(const struct device *dev,
    ...
    *config = (struct behavior_runtime_config){
        ...
        .data.hold_tap =
            {
                .tapping_term_ms = cfg->tapping_term_ms,
                .quick_tap_ms = cfg->quick_tap_ms,
                .require_prior_idle_ms = cfg->require_prior_idle_ms,
                .flavor = cfg->flavor,
                .hold_behavior_dev = cfg->hold_behavior_dev,
                .tap_behavior_dev = cfg->tap_behavior_dev,
            },
    };
...
static int set_hold_tap_runtime_config(const struct device *dev,
    ...
    if (!runtime->hold_behavior_dev || !runtime->tap_behavior_dev || runtime->tapping_term_ms == 0 ||
        ...
        runtime->flavor > BEHAVIOR_HOLD_TAP_FLAVOR_TAP_UNLESS_INTERRUPTED) {
        return -EINVAL;
    }

    if (strcmp(runtime->hold_behavior_dev, dev->name) == 0 ||
        strcmp(runtime->tap_behavior_dev, dev->name) == 0) {
        return -EINVAL;
    }

    if (!zmk_behavior_get_binding(runtime->hold_behavior_dev) ||
        !zmk_behavior_get_binding(runtime->tap_behavior_dev)) {
        return -ENOENT;
    }

    cfg->tapping_term_ms = runtime->tapping_term_ms;
    cfg->quick_tap_ms = runtime->quick_tap_ms;
    cfg->require_prior_idle_ms = runtime->require_prior_idle_ms;
    cfg->flavor = runtime->flavor;
    cfg->hold_behavior_dev = (char *)runtime->hold_behavior_dev;
    cfg->tap_behavior_dev = (char *)runtime->tap_behavior_dev;
...
#endif // IS_ENABLED(CONFIG_ZMK_BEHAVIOR_METADATA)
    .get_runtime_config = get_hold_tap_runtime_config,
    .set_runtime_config = set_hold_tap_runtime_config,
};
