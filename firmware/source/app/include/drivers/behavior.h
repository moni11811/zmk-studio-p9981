...
#define ZMK_BEHAVIOR_RUNTIME_CONFIG_MAX_BINDINGS 8

enum behavior_hold_tap_flavor {
    BEHAVIOR_HOLD_TAP_FLAVOR_HOLD_PREFERRED = 0,
    BEHAVIOR_HOLD_TAP_FLAVOR_BALANCED = 1,
    BEHAVIOR_HOLD_TAP_FLAVOR_TAP_PREFERRED = 2,
    BEHAVIOR_HOLD_TAP_FLAVOR_TAP_UNLESS_INTERRUPTED = 3,
};

enum behavior_runtime_config_type {
    BEHAVIOR_RUNTIME_CONFIG_TYPE_NONE = 0,
    BEHAVIOR_RUNTIME_CONFIG_TYPE_HOLD_TAP = 1,
    BEHAVIOR_RUNTIME_CONFIG_TYPE_TAP_DANCE = 2,
    BEHAVIOR_RUNTIME_CONFIG_TYPE_STICKY_KEY = 3,
};

struct behavior_hold_tap_runtime_config {
...
struct behavior_tap_dance_runtime_config {
    uint32_t tapping_term_ms;
    size_t binding_count;
    struct zmk_behavior_binding bindings[ZMK_BEHAVIOR_RUNTIME_CONFIG_MAX_BINDINGS];
};

struct behavior_sticky_key_runtime_config {
...
struct behavior_runtime_config {
    enum behavior_runtime_config_type type;
    union {
        struct behavior_hold_tap_runtime_config hold_tap;
        struct behavior_tap_dance_runtime_config tap_dance;
        struct behavior_sticky_key_runtime_config sticky_key;
    } data;
};
...
typedef int (*behavior_get_runtime_config_t)(const struct device *behavior,
                                             struct behavior_runtime_config *config);
typedef int (*behavior_set_runtime_config_t)(const struct device *behavior,
...
#endif // IS_ENABLED(CONFIG_ZMK_BEHAVIOR_METADATA)
    behavior_get_runtime_config_t get_runtime_config;
    behavior_set_runtime_config_t set_runtime_config;
};
/**
 * @endcond
 */
...
static inline int behavior_get_runtime_config(const struct device *behavior,
                                              struct behavior_runtime_config *config) {
    if (behavior == NULL || config == NULL) {
        return -EINVAL;
    }

    const struct behavior_driver_api *api = (const struct behavior_driver_api *)behavior->api;
    if (api->get_runtime_config == NULL) {
        return -ENOTSUP;
    }

    return api->get_runtime_config(behavior, config);
}

static inline int behavior_set_runtime_config(const struct device *behavior,
                                              const struct behavior_runtime_config *config) {
    if (behavior == NULL || config == NULL) {
        return -EINVAL;
    }
    ...
    const struct behavior_driver_api *api = (const struct behavior_driver_api *)behavior->api;
    if (api->set_runtime_config == NULL) {
        return -ENOTSUP;
    }

    return api->set_runtime_config(behavior, config);
}
